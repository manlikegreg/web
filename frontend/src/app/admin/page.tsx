...
import AdminDataOverview from '@/components/AdminDataOverview';
import {
  HomeIcon,
  UserGroupIcon,
  DocumentTextIcon,
...
type TabKey = 'dashboard' | 'home' | 'about' | 'students' | 'articles' | 'gallery' | 'contact' | 'leadership' | 'reset';
...
const tabs: { key: TabKey; label: string; icon: any }[] = [
  ...
  { key: 'reset', label: 'Reset Data', icon: TrashIcon },
];
            ...
            <AnimatePresence mode="wait">
              {active === 'dashboard' && (
                <motion.div key="dashboard" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.3 }}>
                  <AdminDataOverview refreshTrigger={refreshTrigger} />
                </motion.div>
              )}
              {active === 'home' && (
              ...
              {active === 'reset' && (
                <motion.div key="reset" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.3 }}>
                  <AdminResetPanel onDataChange={() => {
                    // Refresh data when reset is performed
...
function StudentsAdmin({ toast }: { toast: any }) {
  ...
  const studentCategories = ['Back Benchers','Most Smartest','Quiz Boys','Most Shortest','Most Tallest'];
  const [form, setForm] = useState({ 
    name: '', 
    nickname: '', 
    role: '', 
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      const payload = { 
        ...
        categories: form.categories && form.categories.length ? form.categories : undefined,
      } as any;
    ...
    <div className="space-y-6">
          ...
          <div className="flex flex-wrap items-center gap-2 md:gap-4">
            ...
            <div className="flex flex-wrap justify-center bg-gray-100 rounded-lg p-1">
                  <button
                    ...
                    className={`px-4 py-2 rounded-md font-medium transition-all duration-200 text-sm sm:text-base ${
                      activeTab === 'students'
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {items.map((student) => (
              ...
              >
                ...
                {/* Contact Information */}
                {(student.phone || student.whatsapp || (activeTab === 'teachers' && student.email)) && (
                  <div className="bg-gray-100 rounded-lg p-3 mb-3">
                    <h4 className="text-xs font-semibold text-gray-700 mb-2 uppercase tracking-wide">Contact Info</h4>
      ...
      >
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          {/* Student Categories - only visible for Students tab */}
          {activeTab === 'students' && (
            <div>
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        ...
                        setForm({ ...form, categories: Array.from(set) });
                      }}
                      className={`px-3 py-1 rounded-full text-sm border ${checked ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-700 border-gray-300'}`}
                    >
                      {cat}
                    </button>
                  );
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Bio (Short Summary)</label>
            <textarea
              value={form.bio}
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Detailed description of how this person is in class, their personality, achievements, etc..."
            />
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Additional Contact Info (JSON format)</label>
            <textarea
              value={form.contactInfo}
              onChange={(e) => setForm({ ...form, contactInfo: e.target.value })}
              className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 h-24"
              placeholder='{"instagram": "@username", "facebook": "profile_url", "linkedin": "profile_url"}'
            />
            <p className="text-sm text-gray-500 mt-1">Enter additional contact information as JSON object</p>
          </div>
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
          ...
          {!isBulkDeleteMode ? (
            <>
              <button
                onClick={() => setIsBulkDeleteMode(true)}
                className="px-4 py-2 bg-orange-600 text-white rounded-md hover:bg-orange-700 focus:ring-2 focus:ring-orange-500 focus:ring-offset-2 transition-colors w-full sm:w-auto"
              >
                Bulk Delete
              </button>
              <button
                onClick={startAdd}
                className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-colors w-full sm:w-auto"
              >
                Add Article
              </button>
            </>
            ...
            <>
              <button
                onClick={selectAllItems}
                className="px-4 py-2 bg-gray-600 text-white rounded-md hover:bg-gray-700 focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition-colors w-full sm:w-auto"
              >
                Select All
              </button>
              <button
              ...
              <button
                onClick={bulkDelete}
                disabled={selectedItems.size === 0 || saving}
                className="px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700 focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition-colors disabled:opacity-50 disabled:cursor-not-allowed w-full sm:w-auto"
              >
                {saving ? 'Deleting...' : `Delete Selected (${selectedItems.size})`}
              </button>
              <button
                ...
                className="px-4 py-2 bg-gray-500 text-white rounded-md hover:bg-gray-600 focus:ring-2 focus:ring-gray-400 focus:ring-offset-2 transition-colors w-full sm:w-auto"
              >
                Cancel
              </button>
            </>
          ...
          <div>
            ...
            <div className="flex gap-2 mb-4">
              <input
                ...
                className="flex-1 px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                placeholder="https://example.com/cover-image.jpg or upload a file"
              />
...
function GalleryAdmin({ toast }: { toast: any }) {
  ...
  const [items, setItems] = useState<any[]>([]);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [form, setForm] = useState({ imageUrl: '', caption: '', category: '', type: 'photo' });
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [searchTerm, setSearchTerm] = useState('');
  const [currentPage, setCurrentPage] = useState(1);
  ...
  function startEdit(g: any) {
    setEditingId(g.id);
    setForm({ imageUrl: g.imageUrl || '', caption: g.caption || '', category: (g as any).category || '', type: (g as any).type || 'photo' });
    setIsModalOpen(true);
  }

  function startAdd() {
    setEditingId(null);
    setForm({ imageUrl: '', caption: '', category: '', type: 'photo' });
    setIsModalOpen(true);
  }
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const payload: any = { imageUrl: form.imageUrl, caption: form.caption || undefined, category: form.category || undefined, type: form.type || 'photo' };
